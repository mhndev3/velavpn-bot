...
SETTINGS_TREE = {
    "buttons": {
        ...
        "items": {
            ...
            "btn_referral": ("🎁 دعوت دوستان",       "🎁 دعوت دوستان"),
            "btn_buy_new":  ("⚡ خرید کانفیگ جدید",   "⚡ خرید کانفیگ جدید"),
            "btn_cancel_op": ("❌ لغو عملیات",        "❌ لغو عملیات"),
        },
    },
    "texts": {
        "label": "📝 متن‌های صفحات",
    ...
    "onboarding": {
        ...
        "items": {
            ...
            "onb_username_invalid": ("خطای نام کاربری نامعتبر", "❌ نام کاربری نامعتبر است. یک نام کوتاه‌تر بفرست."),
    ...
    "apps": {
        ...
        "items": {
            ...
            "apps_empty":        ("متن نبود برنامه", "فعلاً برنامه‌ای برای این سیستم‌عامل ثبت نشده."),
        },
    },
    "buy": {
        "label": "🛒 خرید و فاکتور",
        "items": {
            ...
            "buy_name_single_hint": ("راهنمای نام — یک اکانت", "یک نام دلخواه برای کانفیگت بفرست.\nمثلاً: <code>ali-vpn</code>\n\nیا برای ساخت خودکار، دکمهٔ زیر را بزن:"),
            "buy_name_multi_hint": ("راهنمای نام — چند اکانت — {qty}", "برای <b>{qty}</b> اکانت، {qty} نام بفرست — با کاما یا خط جدید جدا کن.\n\nمثال:\n<code>ali-1, ali-2, ali-3</code>\n\n🔹 اگر فقط <b>یک</b> نام بفرستی، بقیه خودکار شماره می‌گیرند (مثلاً <code>ali</code> → <code>ali</code>, <code>ali-2</code>, <code>ali-3</code>)\n🔹 اگر کمتر از {qty} نام بفرستی، بقیه از آخرین نام ساخته می‌شوند\n🔹 یا دکمهٔ زیر را بزن تا برای هر {qty} اکانت نام رندوم ساخته شود"),
            "buy_name_rules":   ("قواعد نام کانفیگ", "📌 <b>قواعد نام:</b>\n• فقط حروف <b>انگلیسی</b> و عدد (فارسی قابل قبول نیست)\n• بدون فاصله — به‌جای فاصله از <code>_</code> یا <code>-</code> استفاده کن\n• بدون علامت‌های خاص مثل <code>@ # $ % / \\ ( )</code>\n• حداکثر ۲۴ کاراکتر"),
            "buy_name_invalid": ("خطای نام نامعتبر", "❌ نام معتبر نبود.\n\nلطفاً فقط از حروف <b>انگلیسی</b> و عدد استفاده کن؛ بدون فاصله و علامت خاص. مثلاً: <code>ali-vpn</code>\n\nیا دکمهٔ «🎲 ساخت نام رندوم» را بزن."),
            "buy_names_show_multi": ("نمایش نام‌های ساخته‌شده — {qty}{names}", "🏷 نام‌های ساخته‌شده ({qty} عدد):\n{names}"),
            "buy_names_autofilled": ("اطلاع تکمیل خودکار نام‌ها — {given}{qty}{names}", "ℹ️ {given} نام فرستادی و {qty} اکانت سفارش دادی؛ بقیه خودکار ساخته شد:\n{names}"),
            "shop_invoice_names_line": ("سطر نام‌ها در فاکتور — {names}", "🏷 نام‌ها: {names}\n"),
    ...
    "renew": {
        ...
        "items": {
            ...
            "rnw_invoice_body": ("بدنه فاکتور — {name}{gb}{dur}", "🏷 کانفیگ: {name}\n📥 حجم: {gb}\n⏳ مدت: {dur}\n♻️ ترافیک ریست می‌شود\n"),
            "rnw_invoice_hint": ("راهنمای فاکتور", "پس از پرداخت، سرویس شما به این حجم و مدت بازنشانی می‌شود و ترافیک مصرف‌شده صفر می‌گردد.\nروش پرداخت را انتخاب کنید:"),
            "rnw_done_title":   ("عنوان پیام موفقیت", "✅ تمدید انجام شد"),
            "rnw_done_body":    ("بدنه پیام موفقیت — {name}{gb}{expiry}", "🏷 کانفیگ: {name}\n📥 حجم کل جدید: {gb}\n♻️ ترافیک مصرف‌شده ریست شد\n📅 انقضای جدید: {expiry}\n\n"),
            "rnw_done_hint":    ("راهنمای پس از تمدید", "لینک کانفیگ شما تغییری نکرده و همان قبلی است. ✅"),
            "rnw_btn_back":     ("دکمهٔ بازگشت", "⬅️ بازگشت"),
            "rnw_config_fallback": ("نام جایگزین کانفیگ", "کانفیگ"),
            "rn_acc_notfound":  ("خطای اکانت پیدا نشد", "اکانت پیدا نشد"),
        },
    },
    "test": {
        "label": "🎁 اکانت تست",
        "items": {
            ...
            "test_ready_body":  ("بدنه اکانت تست — {mb}{hours}{expires}{loc}{link}", "📥 حجم: {mb} مگابایت\n⏳ اعتبار: {hours} ساعت\n📅 انقضا: {expires}\n🌍 لوکیشن: {loc}\n\n🔗 لینک کانفیگ:\n<code>{link}</code>"),
            "test_notfound":    ("خطای اکانت تست یافت نشد", "یافت نشد"),
    ...
    "discount": {
        ...
        "items": {
            ...
            "disc_err_notfound":  ("خطای کد پیدا نشد", "کد تخفیف پیدا نشد."),
            "disc_err_capacity":  ("خطای ظرفیت تمام‌شده", "ظرفیت استفاده از این کد تخفیف تمام شده است."),
            "disc_err_expired":   ("خطای کد منقضی", "این کد تخفیف منقضی شده است."),
        },
    },
    "shop": {
    ...
    "tracking": {
        ...
        "items": {
            ...
            "track_st_imported": ("وضعیت: افزوده‌شده", "افزوده‌شده به اشتراک‌ها"),
        },
    },
    "account": {
        "label": "👤 حساب کاربری",
    ...
    "referral": {
        ...
        "items": {
            ...
            "ref_reward_notify": ("پیام کمیسیون رفرال — {amount}", "🎁 کمیسیون رفرال دریافت کردید!\n\nمبلغ: {amount} تومان\nبه کیف‌پول شما اضافه شد."),
    ...
    "ticket": {
        ...
        "items": {
            "tk_support_intro": ("متن معرفی پشتیبانی", "🛟 پشتیبانی سریع WGV\n━━━━━━━━━━━━━━\n\nبرای پیگیری خرید، مشکل اتصال، تمدید یا راهنمای نصب همینجا تیکت ثبت کنید.\nشماره سفارش، مدل دستگاه و اسکرین‌شات خطا را بفرستید تا بررسی دقیق‌تر انجام شود."),
            "tk_ask_subject":   ("درخواست موضوع تیکت", "موضوع تیکت را کوتاه و واضح ارسال کنید؛ مثلاً: مشکل اتصال V2Ray یا پیگیری سفارش."),
            "tk_btn_back":      ("دکمهٔ بازگشت", "⬅️ بازگشت"),
            "tk_subject_short": ("خطای موضوع کوتاه", "موضوع تیکت خیلی کوتاهه. یه عنوان درست بفرست."),
            "tk_ask_message":   ("درخواست متن پیام", "متن پیام خود را ارسال کنید.\n\nمی‌توانید متن، عکس، ویدیو یا فایل بفرستید."),
            "tk_saved":         ("پیام ثبت تیکت — {ticket_id}", "✅ تیکت شما ثبت شد.\n\nشماره تیکت: <code>{ticket_id}</code>\nپشتیبانی به‌زودی پاسخ می‌دهد."),
        },
    },
    "cfgupdate": {
        "label": "🔄 کانفیگ آپدیت‌شده",
        "items": {
            "cfgu_btn_back":       ("دکمهٔ بازگشت", "⬅️ بازگشت"),
            "cfgu_empty":          ("متن نبود اشتراک", "📦 شما هنوز اشتراک فعالی ندارید.\n\nبرای خرید «⚡ خرید کانفیگ» را بزنید."),
            "cfgu_item_fallback":  ("نام جایگزین اشتراک", "اشتراک"),
            "cfgu_intro":          ("متن معرفی", "🔄 دریافت کانفیگ آپدیت‌شده\n\nاگه پنل فیلتر شده یا IP عوض شده، با این دکمه آخرین کانفیگ رو دریافت کن.\n\nاشتراک خود را انتخاب کنید:"),
            "cfgu_sub_notfound":   ("خطای اشتراک پیدا نشد", "اشتراک پیدا نشد"),
            "cfgu_fetching":       ("پیام در حال دریافت", "⏳ در حال دریافت کانفیگ..."),
            "cfgu_status_current": ("وضعیت: کانفیگ فعلی", "✅ کانفیگ فعلی شما"),
            "cfgu_status_updated": ("وضعیت: آپدیت شد", "✅ کانفیگ آپدیت شد (لینک جدید دریافت شد)"),
            "cfgu_status_same":    ("وضعیت: بدون تغییر", "✅ کانفیگ تغییری نکرده (همان لینک قبلی)"),
            "cfgu_no_config":      ("خطای نبود کانفیگ", "❌ کانفیگی برای این اشتراک ذخیره نشده. با پشتیبانی تماس بگیرید."),
            "cfgu_result":         ("قالب نتیجه — {status}{plan}{expires}", "🔄 {status}\n\n💠 پلن: {plan}\n📅 انقضا: {expires}\n\n"),
            "cfgu_link":           ("سطر لینک — {link}", "🔗 لینک کانفیگ:\n<code>{link}</code>"),
            "cfgu_link_plain":     ("سطر کانفیگ (غیرلینک) — {link}", "🔗 کانفیگ:\n<code>{link}</code>"),
            "cfgu_btn_qr":         ("دکمهٔ QR", "📱 دریافت QR Code"),
            "cfgu_btn_retry":      ("دکمهٔ آپدیت مجدد", "🔄 آپدیت مجدد"),
            "cfgu_qr_notfound":    ("خطای QR", "کانفیگ پیدا نشد"),
            "cfgu_qr_making":      ("پیام ساخت QR", "⏳ ساخت QR..."),
            "cfgu_qr_fallback":    ("لینک جایگزین QR — {link}", "📱 لینک کانفیگ:\n<code>{link}</code>"),
        },
    },
    "addcfg": {
        "label": "➕ افزودن کانفیگ من",
        "items": {
            "addcfg_intro":     ("متن معرفی", "➕ افزودن کانفیگ من\n\nاگر قبلاً کانفیگ گرفته‌ای ولی در «اشتراک‌های من» نیست، کانفیگ (لینک کامل) یا فقط «نام کانفیگ» را بفرست تا پیدا و اضافه‌اش کنم."),
            "addcfg_btn_back":  ("دکمهٔ بازگشت", "⬅️ بازگشت"),
            "addcfg_no_name":   ("خطای نام خوانده نشد", "❌ نام کانفیگ خوانده نشد. لینک کامل یا نام دقیق کانفیگ را بفرست."),
            "addcfg_searching": ("پیام در حال جستجو", "⏳ در حال جستجوی کانفیگ در سرورها..."),
            "addcfg_no_server": ("خطای بدون سرور", "❌ فعلاً سروری تنظیم نشده. با پشتیبانی تماس بگیر."),
            "addcfg_no_panel":  ("خطای اتصال پنل", "❌ در حال حاضر اتصال به پنل برقرار نشد. کمی بعد دوباره تلاش کن یا با پشتیبانی تماس بگیر."),
            "addcfg_not_found": ("خطای کانفیگ پیدا نشد", "❌ کانفیگی با این مشخصات در سرورها پیدا نشد.\n\nبهتر است «لینک کامل» کانفیگ را بفرستی (نه فقط نام) تا با شناسهٔ داخل لینک دقیق پیدا شود.\nاگر باز هم پیدا نشد، با پشتیبانی تماس بگیر."),
            "addcfg_exists":    ("متن کانفیگ تکراری", "ℹ️ این کانفیگ از قبل در «اشتراک‌های من» شما موجود است."),
            "addcfg_done":      ("پیام موفقیت", "✅ کانفیگ پیدا و به «اشتراک‌های من» اضافه شد.\nحالا می‌توانی وضعیت، حجم باقیمانده و کانفیگ آپدیت‌شده‌اش را از همان بخش ببینی."),
        },
    },
    "pricing": {
        "label": "💰 نمایش قیمت (تومان/کریپتو)",
        "items": {
            "price_toman_line":   ("سطر مبلغ تومانی — {price}", "💰 مبلغ تومانی: <b>{price} تومان</b>"),
            "price_crypto_block": ("بلوک معادل کریپتو — {usdt}{trx}", "💵 معادل تقریبی: <b>{usdt} USDT</b>\n🔺 معادل تقریبی: <b>{trx} TRX</b>"),
        },
    },
    "navigation": {
        "label": "🧭 ناوبری و خطاها",
        "items": {
            "glass_section_unavailable": ("خطای بخش غیرقابل‌دسترس", "این بخش در دسترس نیست."),
            "glass_section_error": ("خطای باز کردن بخش", "⚠️ خطا در باز کردن این بخش. لطفاً /start را بزنید."),
        },
    },
    "keyboards": {
